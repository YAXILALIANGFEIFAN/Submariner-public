...
sed -i "s/10.244.0.0/10.144.0.0/g" kube-flannel.yml
...
yq -i eval \
'.clusters[].cluster.server |= sub("10.0.0.12", "10.0.0.12") | .contexts[].name = "cluster-b" | .current-context = "cluster-b"' \
$HOME/.kube/config

sleep 60

kubectl label node vm-0-12-ubuntu  submariner.io/gateway=true
