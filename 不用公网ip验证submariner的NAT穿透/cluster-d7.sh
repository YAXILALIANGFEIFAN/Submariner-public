...
sed -i "s/10.244.0.0/10.88.0.0/g" kube-flannel.yml
...
kubeadm init  \
--apiserver-advertise-address=172.29.100.152 \
--pod-network-cidr=10.88.0.0/16  \
--service-cidr=10.89.0.0/16  \
...
yq -i eval \
'.clusters[].cluster.server |= sub("172.29.100.152", "172.29.100.152") | .contexts[].name = "cluster-c" | .current-context = "cluster-c"' \
$HOME/.kube/config

sleep 60

kubectl label node vm-100-152-ubuntu  submariner.io/gateway=true
kubectl taint nodes --all node-role.kubernetes.io/master-
kubectl create -f kube-flannel.yml

sleep 120

subctl join broker-info.subm --clusterid cluster-d --natt=true
