...
sed -i "s/10.244.0.0/10.4.0.0/g" kube-flannel.yml
...
kubeadm init  \
--apiserver-advertise-address=10.0.0.33 \
...
yq -i eval \
'.clusters[].cluster.server |= sub("10.0.0.33", "10.0.0.33") | .contexts[].name = "cluster-c" | .current-context = "cluster-c"' \
$HOME/.kube/config

sleep 60

kubectl label node vm-0-33-ubuntu  submariner.io/gateway=true
