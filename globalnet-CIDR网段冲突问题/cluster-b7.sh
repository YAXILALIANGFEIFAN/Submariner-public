...
kubeadm init \
	--apiserver-advertise-address=172.29.100.106 \
...
yq -i eval \
'.clusters[].cluster.server |= sub("172.29.100.106", "172.29.100.106") | .contexts[].name = "cluster-b" | .current-context = "cluster-b"' \
$HOME/.kube/config



kubectl label node vm-100-106-ubuntu submariner.io/gateway=true
...
subctl join broker-info.subm --clusterid cluster-b --globalnet-cidr 242.254.145.0/24
# subctl join broker-info.subm --clusterid cluster-b --globalnet-cidr 242.254.145.0/24 --natt=false
