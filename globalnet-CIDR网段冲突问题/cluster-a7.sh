...
kubeadm init \
	...
	--apiserver-cert-extra-sans=localhost,127.0.0.1,10.0.0.11,43.128.72.239 \
...
yq -i eval \
'.clusters[].cluster.server |= sub("10.0.0.11", "43.128.72.239") | .contexts[].name = "cluster-a" | .current-context = "cluster-a"' \
$HOME/.kube/config

# for 1.19.1 this command not need
# kubectl create -f /root/EndpointSlices.yaml

kubectl label node vm-0-11-ubuntu submariner.io/gateway=true
...
subctl deploy-broker --globalnet enabled
# subctl deploy-broker

sleep 60

subctl join broker-info.subm --clusterid cluster-a --globalnet-cidr 242.254.144.0/24
# subctl join broker-info.subm --clusterid cluster-a --globalnet-cidr 242.254.144.0/24 --natt=false
# subctl join broker-info.subm --clusterid cluster-a

scp  broker-info.subm 150.109.241.27:/root
