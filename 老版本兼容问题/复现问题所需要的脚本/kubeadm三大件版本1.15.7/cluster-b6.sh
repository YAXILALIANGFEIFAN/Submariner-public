...
# curl -o /usr/local/bin/calicoctl -O -L https://github.com/projectcalico/calicoctl/releases/download/v3.8.1/calicoctl &&  chmod +x /usr/local/bin/calicoctl
...
kubeadm init \
	--apiserver-advertise-address=10.1.1.48 \
	--pod-network-cidr=10.144.0.0/16  \
	--service-cidr=10.145.0.0/16 \
...
yq -i eval \
'.clusters[].cluster.server |= sub("10.1.1.48", "10.1.1.48") | .contexts[].name = "cluster-b" | .current-context = "cluster-b"' \
$HOME/.kube/config


kubectl create -f /root/EndpointSlices.yaml

kubectl label node vm-1-48-ubuntu submariner.io/gateway=true
...
kubectl create -f /root/cluster2.txt
